...
Generates a structured JSON report from session analysis data.
"""
import os, json, time
...
def build_report(session_id: str, analysis: dict) -> str:
    hosts = analysis.get("risk", {}).get("hosts", analysis.get("context", {}).get("hosts", []))
...
def _build_conclusion(counts: dict, cves: list, target: str) -> str:
    ...
    parts = [f"This assessment of {target or 'the target'} identified an overall risk level of {overall}."]

    if critical_cves:
        parts.append(
            f"{len(critical_cves)} critical CVE(s) were found, including "
            f"{', '.join(c['cve_id'] for c in critical_cves[:3])}."
            " Immediate remediation is required before this system is used in production."
        )
    elif cve_count > 0:
        parts.append(
            f"A total of {cve_count} CVE(s) were mapped to the discovered services. "
            "Patching and version upgrades are recommended."
        )
    else:
        parts.append("No CVEs were matched in this scan. Continue monitoring and run further scan types.")

    parts.append(
        "Apply all patch recommendations, enforce firewall rules, "
        "and schedule a follow-up scan after remediation."
    )
